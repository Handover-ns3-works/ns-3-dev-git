...
def execute_simulation(ttt, hys, speed, angle, qin, qout):
    config = f"--timeToTrigger={ttt} --hysteresis={hys} --speed={speed} --angle={angle} --qin={qin} --qout={qout}"
...
if __name__ == "__main__":
    ...
    hys = [0.5 * i for i in range(0, 31)] if args.hys else [5, 6, 7, 8, 9]
    speed = list(range(10, 101, 10)) if args.speed else [20]
    angle = list(range(10, 91, 10)) if args.angle else [0]

    qin = [i/2 for i in range(-10, 10)] if args.qin else [-3.5]
    qout = [i/2 for i in range(-10, 10)] if args.qout else [-5]
    ...
    with ProcessPoolExecutor() as executor:
                    ...
                    for a in angle:
                        for qi in qin:
                            for qo in qout:
                                sims.append(
                                    executor.submit(
                                        execute_simulation, t, h, s, a, qi, qo
                                    )
                                )
        ...
        try:
            ...
            with open(
                f"{out_dir}{run_name}.csv", "w", newline="", encoding="utf-8"
            ) as csv_file:
...
# in json, output and std error

# ./ns3 configure --build-profile=optimized --out
